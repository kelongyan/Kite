<div align="center">
  ...
  <p>
    <a href="https://github.com/kelongyan/Kite">Source</a>
    ·
    <a href="https://github.com/kelongyan/Kite/releases">Releases</a>
  </p>
</div>

---

Kite is a lightweight open-source terminal (ADE) built on Tauri 2 + Rust and React 19. A native PTY backend with a WebGL renderer, an agentic AI side-panel that runs against your own keys or fully local models, plus a code editor, file explorer, source control with a git graph, and a web preview pane built in. About 7-8 MB on disk. No telemetry. No account.
...
Latest installers are on the [Releases](https://github.com/kelongyan/Kite/releases/latest) page. Auto-update checks are disabled for this fork.

### Windows notes

- On first launch Windows shows "Windows protected your PC" because Kite isn't code-signed yet. Click **More info** then **Run anyway**.
...
- **NixOS / Nix**: use the flake from this repository — `nix profile install github:kelongyan/Kite` (non-NixOS), or import the flake and add `inputs.kite.packages.${pkgs.system}.kite` to `environment.systemPackages` (NixOS).
- **AppImage:** needs FUSE. Without it: `./Kite_*.AppImage --appimage-extract-and-run`. On Wayland with rendering glitches, try `WEBKIT_DISABLE_DMABUF_RENDERER=1`. Otherwise the `.deb` / `.rpm` packages link against the system GTK stack and tend to be smoother.

## Configure AI

1. Open **Settings -> AI**.
2. Pick a provider and paste your API key. For local inference, point Kite at your LM Studio / MLX / Ollama endpoint.
...
Kite is licensed under the Apache-2.0 License. It is based on the upstream Terax project by Crynta. For more information on our dependencies, see [Apache License 2.0](LICENSE).
