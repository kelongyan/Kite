...
export function ProviderKeyCard({
  ...
  const cardMessages = messages.settings.models.providerKeyCard;
  const providerMessages = messages.settings.models.providers;
  ...
  const submit = async () => {
    ...
    if (!trimmed) {
      setError(cardMessages.enterKey);
      return;
    }
    if (provider.keyPrefix && !trimmed.startsWith(provider.keyPrefix)) {
      setError(cardMessages.keyPrefixError(provider.label, provider.keyPrefix));
      return;
    }
    setSaving(true);
    setError(null);
    try {
      ...
      setError(cardMessages.failedToSave(String(e)));
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="flex items-center gap-2">
        ...
        {onRemove ? (
          <Button
            ...
            variant="ghost"
            onClick={onRemove}
            title={cardMessages.removeProvider}
          ...
          <div className="flex gap-1.5">
            <div className="relative flex-1">
              ...
              <button
                ...
                aria-label={
                  reveal ? cardMessages.hideKey : cardMessages.showKey
                }
              >
                <HugeiconsIcon
