...
export function ShortcutsSection() {
  ...
  const shortcutMessages = messages.settings.shortcuts;
  ...
  const filteredShortcuts = useMemo(() => {
    ...
    return base.filter(
      (s) =>
        ...
        shortcutMessages.actions[s.id].toLowerCase().includes(lower) ||
        shortcutMessages.groups[s.group].toLowerCase().includes(lower),
    );
  }, [search, shortcutMessages.actions, shortcutMessages.groups]);
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <SectionHeader
          title={shortcutMessages.title}
          description={shortcutMessages.description}
        />
        <Button
          variant="outline"
          size="sm"
      ...
      <div className="relative">
        ...
        />
        <Input
          placeholder={shortcutMessages.searchPlaceholder}
          value={search}
      ...
      <AlertDialog open={resetDialogOpen} onOpenChange={setResetDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {shortcutMessages.resetDialogTitle}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {shortcutMessages.resetDialogDescription}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
function ShortcutRow({
  ...
  messages: Messages["settings"]["shortcuts"];
}) {
  const bindings =
...
function Recorder({
  ...
  return (
    <div className="flex items-center gap-2 rounded bg-accent/50 px-2 py-1 text-[11px] ring-1 ring-accent">
      <span className="animate-pulse font-medium">{messages.recording}</span>
      <span className="text-muted-foreground">{messages.escToCancel}</span>
    </div>
  );
}
