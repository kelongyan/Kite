  ...
  return (
    <div className="not-prose my-2 flex items-center gap-2 rounded-lg border border-border/50 bg-muted/30 px-3 py-2 text-[11px] text-muted-foreground">
      ...
      <Shimmer duration={1.2}>
        {label === "text"
          ? messages.generatingCode
          : messages.generating(label)}
      </Shimmer>
    </div>
  );
}
...
function RunInTerminalButton({ command }: { command: string }) {
  ...
  return (
    <Button
      ...
      aria-label={messages.runInActiveTerminal}
      title={messages.runInActiveTerminal}
    >
      <HugeiconsIcon
        ...
        strokeWidth={1.75}
      />
      <span>{sent ? messages.sent : messages.run}</span>
    </Button>
  );
}
