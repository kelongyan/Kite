...
const TOOL_META: Record<string, { label: string; icon: typeof File01Icon }> = {
  read_file: { label: "read_file", icon: File01Icon },
  list_directory: { label: "list_directory", icon: FolderOpenIcon },
  write_file: { label: "write_file", icon: FilePlusIcon },
  create_directory: { label: "create_directory", icon: FolderAddIcon },
  edit: { label: "edit", icon: FileEditIcon },
  multi_edit: { label: "multi_edit", icon: Edit02Icon },
  bash_run: { label: "bash_run", icon: TerminalIcon },
  bash_background: { label: "bash_background", icon: TerminalIcon },
  bash_logs: { label: "bash_logs", icon: TerminalIcon },
  bash_list: { label: "bash_list", icon: TerminalIcon },
  bash_kill: { label: "bash_kill", icon: TerminalIcon },
  grep: { label: "grep", icon: GlobalSearchIcon },
  glob: { label: "glob", icon: Folder01Icon },
  suggest_command: { label: "suggest_command", icon: SparklesIcon },
  open_preview: { label: "open_preview", icon: EyeIcon },
  run_subagent: { label: "run_subagent", icon: RobotIcon },
  todo_write: { label: "todo_write", icon: CheckListIcon },
};
...
const ToolImpl = ({
  ...
  const messages = useMessages().ai.tool;
  const meta = TOOL_META[toolName];
  const Icon = meta?.icon ?? ToolsIcon;
  const label =
    meta?.label && meta.label in messages.labels
      ? messages.labels[meta.label as keyof typeof messages.labels]
      : toolName;
  const rawSummary = deriveSummary(toolName, input);
  const summary =
    toolName === "todo_write" && rawSummary
      ? messages.itemCount(Number(rawSummary))
      : rawSummary;
    ...
    >
      ...
      >
        <span
          ...
          aria-label={statusLabel(state, messages)}
        />
        <HugeiconsIcon
          icon={Icon}
          size={13}
...
function ToolOutput({
  ...
  const custom = renderToolOutput(toolName, output, messages);
...
function renderToolOutput(
  toolName: string,
  output: unknown,
  messages: ReturnType<typeof useMessages>["ai"]["tool"],
): ReactNode | null {
  ...
  if (toolName === "read_file") {
    ...
    return (
      <div className="flex items-center gap-1.5 font-mono text-[11px]">
        ...
        <span className="text-foreground">{messages.read}</span>
        {path ? <span className="text-muted-foreground">· {path}</span> : null}
        {lines != null ? (
          <span className="text-muted-foreground">
            ({messages.lineCount(lines)}
  ...
  if (toolName === "grep") {
    ...
    if (hits.length === 0) {
      return (
        <div className="text-[11px] italic text-muted-foreground">
          {messages.noMatches}
          {filesScanned != null ? ` · ${messages.filesScanned(filesScanned)}` : ""}
        </div>
      );
    }
    ...
    return (
      <div className="space-y-1">
        ...
        <div className="flex items-center justify-between text-[10px] text-muted-foreground">
          <span>
            {messages.hitCount(hits.length)}
            {filesScanned != null ? ` · ${messages.filesCount(filesScanned)}` : ""}
          </span>
          {truncated ? (
        ...
        <div className="flex items-center gap-1.5 font-mono text-[11px]">
          ...
          {reps != null ? (
            <span className="text-foreground">
              {messages.replacementCount(reps)}
            </span>
          ) : null}
          {path ? (
  ...
  if (toolName === "write_file" || toolName === "create_directory") {
    ...
    return (
      <div className="flex items-center gap-1.5 font-mono text-[11px]">
        <span className="text-emerald-600 dark:text-emerald-400">✓</span>
        <span className="text-foreground">
          {toolName === "create_directory" ? messages.created : messages.wrote}
        </span>
        {path ? <span className="text-muted-foreground">· {path}</span> : null}
        {bytes != null ? (
    ...
    return (
      <div className="space-y-0.5 font-mono text-[11px]">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-muted-foreground">{messages.running}</span>
        </div>
        {cmd ? (
    ...
    <div className="space-y-1.5">
      ...
      <div className="flex items-stretch gap-1.5 rounded bg-muted/40 overflow-hidden">
        ...
        <button
          ...
          aria-label={messages.insertIntoActiveTerminal}
        >
          <HugeiconsIcon
            ...
            strokeWidth={1.75}
          />
          <span>{inserted ? messages.inserted : messages.insert}</span>
        </button>
      </div>
    </div>
  );
...
function statusLabel(
  ...
  switch (state) {
    case "approval-requested":
      return messages.status.approvalRequested;
    case "approval-responded":
      return messages.status.approvalResponded;
    case "input-streaming":
      return messages.status.inputStreaming;
    case "input-available":
      return messages.status.inputAvailable;
    case "output-available":
      return messages.status.outputAvailable;
    case "output-denied":
      return messages.status.outputDenied;
    case "output-error":
      return messages.status.outputError;
  }
}
