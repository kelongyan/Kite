...
export const ReasoningTrigger = memo(
  ({
    ...
    const messages = useMessages().ai.reasoning;
    const { isStreaming, isOpen, duration } = useReasoning();
    const message =
      getThinkingMessage?.(isStreaming, duration) ??
      defaultThinkingMessage(isStreaming, duration, messages);
...
function defaultThinkingMessage(
  isStreaming: boolean,
  duration: number | undefined,
  messages: ReturnType<typeof useMessages>["ai"]["reasoning"],
): ReactNode {
  if (isStreaming || duration === 0) {
    return <Shimmer duration={1}>{messages.thinking}</Shimmer>;
  }
  if (duration === undefined) {
    return <span>{messages.reasoned}</span>;
  }
  return <span>{messages.reasonedFor(duration)}</span>;
}
