...
export function WorkspaceInputBar({
  ...
  const messages = useMessages().mainShell.workspaceInput;
  const c = useComposer();
  ...
  const terminalChips = isTerminalTab ? (
    <>
      ...
      {branch && (
        <Chip tone="violet" icon={GitBranchIcon} title={messages.branch(branch)}>
          {branch}
        </Chip>
      )}
      {shell && (
