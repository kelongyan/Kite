...
import { messagesFor, type AppLanguage } from "@/modules/i18n";
...
function route(
  ...
  const messages = messagesFor(ctx.language).ai.notifications;
  const heading =
    kind === "attention"
      ? messages.agentNeedsInput(name)
      : messages.agentFinished(name);
...
export function AgentNotificationsBridge({
  ...
  const language = usePreferencesStore((s) => s.appLanguage);
  const ctxRef = useRef<Ctx>({ tabs, activeId, focused, language, onActivate });
  ctxRef.current = { tabs, activeId, focused, language, onActivate };
