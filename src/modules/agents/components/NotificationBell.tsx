...
function relativeTime(
  ts: number,
  messages: ReturnType<typeof useMessages>["ai"]["notifications"],
): string {
  const s = Math.floor((Date.now() - ts) / 1000);
  if (s < 60) return messages.justNow;
  const m = Math.floor(s / 60);
  if (m < 60) return messages.minutesAgo(m);
  const h = Math.floor(m / 60);
  if (h < 24) return messages.hoursAgo(h);
  return messages.daysAgo(Math.floor(h / 24));
}
  ...
  return (
    ...
    >
      ...
      >
        ...
        {waiting ? messages.waiting : messages.working}
      </span>
    </button>
  );
}
    ...
    <div className="flex items-center gap-2 px-2 py-1">
      ...
      {ready ? (
        ...
        >
          ...
          {installing ? messages.enabling : messages.enable}
        </button>
      )}
    </div>
  );
    ...
    >
      ...
      <span className="min-w-0 flex-1 truncate text-sm text-foreground">
        ...
        <span className="text-muted-foreground">
          {messages.labels[n.kind]}
        </span>
      </span>
      <span className="shrink-0 text-[10px] tabular-nums text-muted-foreground">
        {relativeTime(n.at, messages)}
      </span>
    </button>
  );
}
          ...
          <div className="ml-auto flex items-center gap-2">
            {activeCount > 0 ? (
              <span className="rounded-full bg-accent px-1.5 py-0.5 text-[10px] font-medium tabular-nums text-muted-foreground">
                {messages.activeCount(activeCount)}
              </span>
            ) : null}
