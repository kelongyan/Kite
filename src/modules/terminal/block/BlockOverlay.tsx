...
import { useMessages, type Messages } from "@/modules/i18n";import {
  ArrowDown01Icon,
  ArrowUp01Icon,
  Cancel01Icon,
  Clock01Icon,
...
type ChromeProps = {
  ...
  messages: Messages["mainShell"]["terminalBlocks"];
};

// No chrome while the command runs; the bar lands together with the divider
// once the block is finished.
function BlockChrome({ block, all, onSearch, messages }: ChromeProps) {
...
function StickyHeader({ block, all, onSearch, messages }: ChromeProps) {
  return (
    <div className="bt-sticky">
      ...
      <span className="bt-sticky-cmd">
        {block.command || messages.commandFallback}
      </span>
      <Toolbar
        block={block}
        all={all}
...
function Toolbar({ block, all, onSearch, messages }: ChromeProps) {
...
function BlockMenu({ block, all, onSearch, messages }: ChromeProps) {
  const output = () => all.readOutput(block.id) ?? "";
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button type="button" title={messages.blockActions} className="bt-btn">
          <HugeiconsIcon
      ...
      >
        ...
        <MenuItem
          ...
          onClick={() => copy(block.command, messages.commandCopied)}
        />
        <MenuItem
          icon={ComputerTerminal02Icon}
          label={messages.copyOutput}
          onClick={() => {
            const o = output();
            if (o) copy(o, messages.outputCopied);
          }}
        />
        <MenuItem
          icon={Copy01Icon}
          label={messages.copyCommandAndOutput}
          onClick={() => {
            const text = `$ ${block.command}\n${output()}`;
            copy(text, messages.blockCopied);
          }}
        />
        <MenuItem
          icon={Search01Icon}
          label={messages.findInBlock}
  ...
  return (
    <div className="bt-search pointer-events-auto">
      ...
      <input
        ...
        placeholder={messages.findInBlock}
      ...
      <SearchBtn
        ...
        onClick={() => nav(1)}
      />
      <SearchBtn title={messages.close} icon={Cancel01Icon} onClick={onClose} />
    </div>
  );
}
