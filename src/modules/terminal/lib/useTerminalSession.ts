...
type Session = {
  ...
  // OSC 133 C..D window: a foreground process owns
...
async function leafHasForegroundJob(leafId: number): Promise<boolean> {
  ...
  try {
    ...
    console.error("[kite] pty_has_foreground_job failed for leaf", leafId, e);
    return false;
  }
}
    ...
    return {
      ...
      kickPty: (cols, rows) => {
        ...
        pty
          ...
          .catch((e) => console.warn("[kite] kickPty failed:", e));
      },
    };
  },
  evictLeaf(leafId) {
...
async function openPtyWithRetry(
  ...
  try {
    ...
    console.error("[kite] openPty failed, retrying once:", e);
...
function surfaceSpawnFailure(leafId: number, s: Session, e: unknown): void {
  console.error("[kite] shell spawn failed:", e);
  s.shellExited = true;
  s.spawnFailed = true;
  ...
  deliverPtyBytes(
    ...
    new TextEncoder().encode(
      `\r\n\x1b[31m[kite] failed to start shell: ${detail}\x1b[0m\r\n\x1b[2mpress Enter to retry\x1b[0m\r\n`,
    ),
  );
}
...
export async function leafHasForegroundProcess(
  ...
  try {
    ...
    console.error(
      "[kite] pty_has_foreground_process failed for leaf",
      leafId,
      e,
    );
    return false;
...
export function useTerminalSession({
  ...
  useEffect(() => {
    const s = ensureSession(leafId, initialCwdRef.current, themeMode);
