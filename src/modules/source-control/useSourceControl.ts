      ...
      try {
        ...
        if (shouldAutoFetch) {
          try {
            ...
            nextRemoteError = normalizeError(error, messages.unknownError);
          }
        }
        ...
        setState((current) => ({
          ...
          status: null,
          isLoading: false,
          localError: normalizeError(error, messages.unknownError),
        }));
      } finally {
        lastRefreshAtRef.current = Date.now();
      }
    },
    [contextPath, messages.unknownError, workspaceKey],
  );
  ...
  const runRemoteAction = useCallback(
    async (
      ...
      try {
        ...
        const message = normalizeError(error, messages.unknownError);
