...
function normalizeError(
  error: unknown,
  fallback = "Unknown source control error",
): string {
...
export function useSourceControlPanel(
  ...
  const aiUnavailableReason = useMemo(() => {
    if (stagedEntries.length === 0) {
      return messages.stageChangesToGenerate;
    }
    if (!hasApiKeyForSelected) {
      return messages.connectAiProvider;
    }
  ...
  const generateCommitMessageHint = aiUnavailableReason
    ? aiUnavailableReason
    : aiBusy
      ? messages.waitForAiAction
      : messages.generateCommitMessage;
  const pushHint = useMemo(() => {
    if (!status) return null;
    if (!status.upstream) {
      return messages.configureBranchForPush;
    }
    if (status.behind > 0) {
      return messages.pullBeforePush;
    }
    if (status.ahead === 0) {
      return messages.noLocalCommitsToPush(status.upstream);
    }
    return messages.pushesTo(status.upstream);
  }, [messages, status]);
  const stagedEmptyText = messages.noStagedChanges;
  const unstagedEmptyText = messages.noUnstagedChanges;
  ...
  const runMutation = useCallback(
    async (
      ...
      try {
        ...
        setActionError(normalizeError(error, messages.unknownError));
        cancelReconcile();
    ...
    [cancelReconcile, messages.unknownError, repo, scheduleReconcile, summary],
  );
  ...
  const generateCommitMessage = useCallback(async () => {
    if (!repo || stagedEntries.length === 0) return;
    if (aiBusy) {
      setActionError(messages.waitForAiAction);
      return;
    }
    ...
    try {
      ...
      if (!isValidCommitMessage(message)) {
        throw new Error(messages.invalidAiCommitMessage);
      }
    ...
    openaiCompatibleModelId,
    openrouterModelId,
    messages.invalidAiCommitMessage,
  ...
  const commit = useCallback(async () => {
    ...
    try {
      ...
      setCommitMessage("");
      setActionMessage(
        messages.committed(result.commitSha.slice(0, 7), result.summary),
      );
  ...
  }, [commitMessage, messages, repo, summary]);
  ...
  const push = useCallback(async () => {
    ...
    if (result.ok) {
      setActionMessage(
        status?.upstream
          ? messages.pushedTo(status.upstream)
          : messages.pushCompleted,
      );
      return;
    }
    if (result.error) {
  ...
  }, [messages, repo, status?.upstream, summary]);
  ...
  const pendingDiscardView = useMemo<PendingDiscard | null>(() => {
    ...
    return {
      ...
      label: messages.unstagedFiles(pendingDiscard.entries.length),
    };
  }, [messages, pendingDiscard]);
