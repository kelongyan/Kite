...
function BranchDropdown({
  ...
  onNavigateToPath?: (path: string) => void;
  onRefresh: () => void;
  messages: ReturnType<typeof useMessages>["workspace"]["sourceControl"];
}) {
...
export const SourceControlPanel = memo(function SourceControlPanel({
  ...
  const messages = useMessages().workspace.sourceControl;
  ...
  const repoLabel = useMemo(() => {
    if (!scm.status) return messages.title;
    return scm.status.isDetached ? "detached" : scm.status.branch;
  }, [messages.title, scm.status]);
  ...
  const commitDisabledReason = scm.actionBusy
    ? messages.waitForGitAction
    : scm.stagedEntries.length === 0
      ? messages.stageChangesToCommit
      : scm.commitMessage.trim().length === 0
        ? messages.enterCommitMessage
        : null;
  const commitHint = canCommit
    ? messages.commitWithShortcut(commitShortcut)
    : (commitDisabledReason ?? messages.commitWithShortcut(commitShortcut));
  const pushHint = scm.pushHint ?? messages.pushUnavailable;
  ...
  return (
    <TooltipProvider delayDuration={800} skipDelayDuration={300}>
      <aside className="flex h-full min-w-0 flex-col bg-card/80 backdrop-blur [contain:layout_style]">
        <header className="flex shrink-0 items-center justify-between gap-2 border-b border-border/50 px-3 pb-2.5 pt-3">
          ...
          <div className="flex shrink-0 items-center gap-0.5">
            <IconActionButton
              label={fetchBusy ? messages.fetching : messages.fetchFromRemote}
              disabled={!canFetch}
              onClick={handleFetch}
              side="bottom"
            >
            ...
            <IconActionButton
              label={
                pullBusy
                  ? messages.pulling
                  : isDiverged
                    ? messages.branchDivergedResolve
                    : !hasUpstream
                      ? messages.noUpstreamConfigured
                      : (scm.status?.behind ?? 0) === 0
                        ? messages.alreadyUpToDate
                        : messages.pullCommits(scm.status?.behind ?? 0)
              }
              disabled={!canPull}
              onClick={handlePull}
              side="bottom"
            ...
            <IconActionButton
              label={messages.refreshSourceControl}
        ...
        {scm.panelState === "no-repo" ? (
          <PanelCenter
            title={messages.noRepository}
            body={messages.noRepositoryBody}
          />
        ) : null}

        {scm.panelState === "error" ? (
          <PanelCenter
            title={messages.sourceControlError}
            body={scm.statusError ?? messages.unknownError}
            action={
        ...
        {scm.panelState === "ready" && scm.status ? (
          <>
            <div className="relative shrink-0 space-y-2 border-b border-border/40 bg-gradient-to-b from-card/65 to-card/30 px-2.5 pb-2.5 pt-2.5">
              ...
              >
                <Textarea
                  ...
                  placeholder={messages.commitMessagePlaceholder}
                  rows={3}
                  className={cn(
              ...
              <div className="flex min-w-0 items-center gap-1.5 text-[10.5px] text-muted-foreground">
                ...
                <span className="truncate font-medium text-foreground/85">
                  {stagedCount === 0
                    ? messages.nothingStaged
                    : messages.stagedCount(stagedCount)}
                </span>
            ...
            {scm.allClean ? (
              <CleanTreeHint repoLabel={repoLabel} messages={messages} />
            ) : (
              <div
                ref={containerRef}
                tabIndex={0}
                role="listbox"
                aria-label={messages.changedFiles}
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{messages.discardChangesTitle}</AlertDialogTitle>
            <AlertDialogDescription>
              {scm.pendingDiscard?.scope === "all"
                ? messages.discardAllDescription(scm.pendingDiscard.label)
                : scm.pendingDiscard
                  ? messages.discardSingleDescription(scm.pendingDiscard.label)
                  : null}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
const RowRenderer = memo(function RowRenderer(props: RowRendererProps) {
  ...
  switch (row.kind) {
    case "banner-diverged":
      return <DivergedBanner messages={props.messages} />;
    case "list-header":
  ...
  return (
    <div className="flex h-7 items-center gap-2 px-3">
      ...
      <label className="ml-auto flex shrink-0 cursor-pointer select-none items-center gap-1.5 text-[10.5px] font-medium text-muted-foreground hover:text-foreground">
        ...
        <Checkbox
          aria-label={messages.stageAllChanges}
  ...
  return (
    <ContextMenu>
        ...
        >
          ...
          {showDiscard ? (
            <div className="flex shrink-0 items-center opacity-0 transition-opacity group-hover:opacity-100 data-[focused=true]:opacity-100 data-[selected=true]:opacity-100">
              <IconActionButton
                label={messages.discardPath(entry.path)}
                disabled={disabled}
                side="top"
          ...
          <span className="flex size-5 shrink-0 items-center justify-center">
            {isStageBusy ? (
              ...
              <Checkbox
                aria-label={messages.stagePath(entry.path)}
      ...
      <ContextMenuContent className={COMPACT_CONTENT}>
        ...
        >
          {entry.checkState === "checked" ? messages.unstage : messages.stage}
        </ContextMenuItem>
        {entry.unstaged ? (
          <ContextMenuItem
            className={COMPACT_ITEM}
