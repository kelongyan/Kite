...
export function SpaceSwitcher({
  ...
  const messages = useMessages().mainShell.spaces;
  ...
  return (
    <Popover open={open} onOpenChange={onOpenChange}>
      <PopoverTrigger asChild>
        <button
          type="button"
          title={shortcut ? `${messages.title} · ${shortcut}` : messages.title}
      ...
      <PopoverContent align="start" sideOffset={6} className="w-[20rem] p-1.5">
        ...
        <div className="mt-1.5 border-t border-border/60 pt-1.5">
          ...
          >
            ...
            <span className="flex-1">{messages.newSpace}</span>
          </button>
        </div>
      </PopoverContent>
      {overlay &&
...
type SpaceRowProps = {
  ...
  onJumpTab: (id: number) => void;
  onCloseTab: (id: number) => void;
  messages: Messages["mainShell"]["spaces"];
};
          ...
          <>
            ...
            >
              <RowAction
                ...
                label={messages.renameSpace}
                onClick={onStartRename}
              />
              <RowAction
                icon={PlusSignIcon}
                label={messages.newTab}
                onClick={onNewTab}
              />
              {canDelete && (
                <RowAction
                  icon={Delete02Icon}
                  label={messages.deleteSpace}
                  destructive
                  onClick={onDelete}
                />
              )}
        ...
        <div className="flex flex-col gap-px py-0.5 pl-10 pr-0.5">
          ...
          {tabs.length === 0 && (
            <span className="px-2 py-1 text-[10.5px] text-muted-foreground/50">
              {draggingTabFromOther ? messages.dropToMoveHere : messages.noTabs}
            </span>
          )}
        </div>
      )}
