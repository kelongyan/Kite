...
export function useExplorerFileDrop({ rootPath, isDir, onCopied }: Options) {
  ...
  const optsRef = useRef({ rootPath, isDir, onCopied, messages });
  optsRef.current = { rootPath, isDir, onCopied, messages };
  ...
  useEffect(() => {
    ...
    void getCurrentWebview()
      .onDragDropEvent((e) => {
        const p = e.payload;
        const { rootPath, isDir, onCopied, messages } = optsRef.current;
        ...
        if (p.type === "drop") {
          ...
          void invoke("fs_copy", {
            ...
            .then(() => onCopied(dir))
            .catch((err) => toast.error(messages.copyFailed(String(err))));
        }
      })
      .then((fn) => {
        if (disposed) fn();
      ...
      .catch((err) => console.error("[kite] explorer drop listen failed:", err));
