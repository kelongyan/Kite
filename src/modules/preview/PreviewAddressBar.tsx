...
export const PreviewAddressBar = forwardRef<PreviewAddressBarHandle, Props>(
  function PreviewAddressBar({ url, onSubmit, onReload }, ref) {
    const messages = useMessages().workspace.preview;
    ...
    const submit = () => {
      ...
      if (!next) {
        setNotice(messages.enterUrlOrPort);
        return;
      }
      setNotice(null);
    ...
    const tryPort = async (port: number) => {
      ...
      if (!ok) {
        setNotice(messages.noServerListening(port));
        return;
      }
      setDraft(url);
      onSubmit(url);
    ...
    return (
      ...
      <div className="flex h-9 items-center gap-1 bg-card/40 px-1.5">
          ...
          <DropdownMenuTrigger asChild>
            ...
            >
              <HugeiconsIcon
                ...
                strokeWidth={1.75}
              />
              <span className="hidden sm:inline">{messages.ports}</span>
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent
            align="start"
            ...
            {PORT_PRESETS.map((p) => (
              ...
              >
                ...
                <span className="text-xs text-muted-foreground">
                  {checkingPort === p.port ? messages.checking : `:${p.port}`}
                </span>
              </DropdownMenuItem>
            ))}
          </DropdownMenuContent>
        ...
        <Button
          ...
          title={messages.openInSystemBrowser}
          className="size-7 shrink-0 rounded-md text-muted-foreground hover:bg-accent hover:text-foreground"
          disabled={!url}
        >
          <HugeiconsIcon
