...
export function GitHistoryPane({
  ...
  const messages = useMessages().workspace.gitHistory;
  ...
  const loadInitial = useCallback(async () => {
    ...
    try {
      ...
      if (requestId !== requestIdRef.current) return;
      setError(normalizeError(err, messages.unknownError));
      setLoadStatus("error");
    }
  }, [messages.unknownError, repoRoot]);
  ...
  }, [commits, endReached, loadStatus, messages.unknownError, repoRoot]);
  ...
  const fetchFiles = useCallback(
    async (sha: string) => {
      ...
      try {
        ...
        cache.set(sha, {
          state: "error",
          error: normalizeError(err, messages.unknownError),
        });
        bumpFiles();
      } finally {
    ...
    [bumpFiles, messages.unknownError, repoRoot],
  );
        ...
        {loadStatus === "initial" && commits.length === 0 ? (
          <CenterPlaceholder>
            ...
            <span className="text-[11.5px] text-muted-foreground">
              {messages.loadingCommits}
            </span>
          </CenterPlaceholder>
        ) : loadStatus === "error" && commits.length === 0 ? (
          <CenterPlaceholder>
            <div className="text-[13px] font-medium">
              {messages.couldNotLoadHistory}
            </div>
          ...
          <CenterPlaceholder>
            <div className="text-[13px] font-medium">
              {messages.noCommitsYet}
            </div>
            <div className="max-w-md text-[11px] leading-relaxed text-muted-foreground">
              {messages.branchHasNoCommits}
            </div>
          </CenterPlaceholder>
        ) : (
          <>
            ...
            >
              <div />
              <div className="pl-px">{messages.sha}</div>
              <div className="min-w-0">{messages.subject}</div>
              <div />
              <div className="ml-2">{messages.author}</div>
              <div className="text-right">{messages.date}</div>
              <div className="text-right">{messages.changes}</div>
            </div>
            <div
              ref={scrollRef}
            ...
            >
              ...
              {loadStatus === "error" && commits.length > 0 ? (
                <div className="flex items-center justify-center gap-2 py-3 text-[11px] text-destructive">
                  {error ?? messages.failedToLoadMore}
                  <Button
                    size="xs"
                    variant="ghost"
...
type CommitRowProps = {
  ...
  messages: ReturnType<typeof useMessages>["workspace"]["gitHistory"];
};
    ...
    >
      ...
      >
        {commit.subject ? (
          ...
          <span className="text-muted-foreground">{messages.noSubject}</span>
        )}
      </span>
      <span aria-hidden />
      <span
      ...
      >
        ...
        <span className="min-w-0 truncate">
          {commit.author ? highlight(commit.author, query) : messages.unknown}
        </span>
      </span>
      ...
      <span className="flex min-w-0 items-center justify-end gap-1.5 font-mono text-[10px] tabular-nums">
        {commit.filesChanged > 0 ? (
          <span
            ...
            title={messages.filesChanged(commit.filesChanged)}
          >
            <HugeiconsIcon
              icon={File02Icon}
              size={10.5}
    ...
    <div className="flex max-h-[60vh] min-h-0 flex-col">
      <div className="shrink-0 border-b border-border/45 p-3">
        ...
        <div className="mt-2 flex min-w-0 items-center gap-1.5 text-[10.5px] text-muted-foreground">
          <span className="truncate">{commit.author || messages.unknown}</span>
          {commit.authorEmail ? (
            <>
        ...
        <div className="mt-2.5 flex items-center gap-1">
          ...
          >
            ...
            {copied ? messages.copied : messages.copySha}
          </Button>
          {webUrl ? (
            <Button
              size="xs"
  ...
  return (
    ...
    >
      ...
      <div className="flex shrink-0 items-center gap-1 text-[10px] tabular-nums">
        {file.isBinary ? (
          <span className="text-muted-foreground/70">{messages.binary}</span>
        ) : (
          <>
            {file.added > 0 ? (
