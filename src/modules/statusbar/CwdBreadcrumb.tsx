...
function CurrentSegmentDropdown({
  ...
  label: string;
  path: string;
  messages: Messages["mainShell"]["statusbar"]["cwd"];
  onCd: (p: string) => void;
}) {
      ...
      <BreadcrumbItem>
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button
              type="button"
              title={messages.showParentFolders}
            ...
            {segments.map((s) => (
              ...
              >
                ...
                <span className="truncate">
                  {s.isHome ? messages.home : s.label}
                </span>
              </DropdownMenuItem>
            ))}
          </DropdownMenuContent>
