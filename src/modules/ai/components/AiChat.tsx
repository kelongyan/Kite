...
function CommandSnippet({ name }: { name: string }) {
  const labels = useMessages().ai.slashCommands;
  ...
  return (
    <div className="inline-flex max-w-full items-center gap-2 rounded-md border border-border/50 bg-muted/40 px-2 py-1">
      ...
      <span className="truncate text-[11px] text-muted-foreground">
        {name in labels ? labels[name as keyof typeof labels] : meta.label}
      </span>
    </div>
  );
}
      ...
      {chips.map((c, i) => (
        ...
        >
          ...
          <span className="font-medium text-foreground">
            {chipLabel(c, messages)}
          </span>
          {"lines" in c && c.lines > 0 ? (
            <span className="opacity-70">· {messages.linesShort(c.lines)}</span>
          ) : null}
        </span>
      ))}
    </div>
...
function chipLabel(
  ...
  if (c.kind === "selection") {
    return c.source === "editor"
      ? messages.editorSelection
      : messages.terminalSelection;
  }
...
export function AiChatView({
  ...
  const messagesText = useMessages().ai.chat;
  ...
  if (messages.length === 0) {
    return (
      <Conversation>
        <ConversationContent>
          <ConversationEmptyState
            title={messagesText.emptyTitle}
            description={messagesText.emptyDescription}
          />
        </ConversationContent>
      </Conversation>
    );
      ...
      <ConversationContent className="gap-5 p-3">
        ...
        {showSpinner && (
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            <Spinner />
            <span className="truncate">{step ?? messagesText.thinking}</span>
          </div>
        )}
        {showContinue && (
          <ContinueRow
        ...
        {error && (
          <div className="rounded-md border border-destructive/40 bg-destructive/10 px-3 py-2 text-xs text-destructive">
            <div className="font-medium">{messagesText.errorTitle}</div>
  ...
  return (
    <div className="flex items-center gap-2 rounded-md border border-border/40 bg-muted/30 px-2.5 py-1.5 text-[11px] text-muted-foreground">
      ...
      <span className="flex-1 truncate">
        {messages.contextCompacted(droppedCount)}
      </span>
      <button
        type="button"
        onClick={onDismiss}
