...
const SUGGESTION_ICONS = {
  explainError: AlertCircleIcon,
  generateCommand: TerminalIcon,
  summarizeBuffer: FilterIcon,
} as const;
...
function sessionDisplayTitle(
  ...
  return session.title === "New chat" ? untitledSession : session.title;
}
...
function PlanModeStrip() {
  const messages = useMessages().ai.mini;
  ...
  return (
    <div className="flex shrink-0 items-center gap-2 border-b border-border/40 bg-muted/40 px-3 py-1.5">
      ...
      <span className="text-[11px] text-muted-foreground">
        · {queueLen > 0 ? messages.queued(queueLen) : messages.noEditsQueued}
      </span>
...
function Header({
  ...
  const uiMessages = useMessages().ai;
    ...
    >
      ...
      <div className="flex shrink-0 items-center gap-1">
        ...
        <Button
          ...
          onClick={onClose}
          className="size-5"
          aria-label={uiMessages.mini.close}
          title={uiMessages.mini.closeEsc}
        >
...
function ContextIndicator({ messages }: { messages: UIMessage[] }) {
  const localeMessages = useMessages().ai.mini.context;
      ...
      <ContextContent className="w-64 text-[11px]">
        ...
        <ContextContentBody>
          <div className="flex items-center justify-between text-muted-foreground">
            <span>{localeMessages.model}</span>
            <span className="font-mono text-foreground">{modelLabel}</span>
          </div>
          <div className="mt-1 flex items-center justify-between text-muted-foreground">
            <span>
              {lastInput > 0
                ? localeMessages.lastRequest
                : localeMessages.estimatedContext}
            </span>
          ...
          {lastCached > 0 && (
            <div className="flex items-center justify-between text-muted-foreground">
              <span>{localeMessages.ofWhichCached}</span>
              <span className="font-mono text-foreground">
          ...
          {reported > 0 && (
            <>
              <div className="mt-1.5 flex items-center justify-between text-muted-foreground">
                <span>{localeMessages.sessionInput}</span>
                <span className="font-mono text-foreground">
                  {formatTokens(tokens.inputTokens)}
                </span>
              </div>
              <div className="flex items-center justify-between text-muted-foreground">
                <span>{localeMessages.sessionOutput}</span>
                <span className="font-mono text-foreground">
              ...
              {tokens.cachedInputTokens > 0 && (
                <div className="flex items-center justify-between text-muted-foreground">
                  <span>{localeMessages.cacheHit}</span>
                  <span className="font-mono text-foreground">{cacheRate}%</span>
                </div>
              )}
              {cost != null && (
                <div className="flex items-center justify-between text-muted-foreground">
                  <span>{localeMessages.sessionCost}</span>
                  <span className="font-mono text-foreground">
          ...
          <div className="flex items-center justify-between text-muted-foreground">
            <span>{localeMessages.window}</span>
            <span className="font-mono text-foreground">
              {formatTokens(max)}
            </span>
          </div>
        ...
        <ContextContentFooter>
          <span className="text-[10px] italic text-muted-foreground">
            {lastInput > 0
              ? localeMessages.lastRequestNote
              : localeMessages.approximateNote}
          </span>
        </ContextContentFooter>
      </ContextContent>
    </Context>
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          ...
          title={messages.switchSession}
        >
          <span className="truncate">
            {sessionDisplayTitle(active, messages.untitledSession)}
          </span>
          <HugeiconsIcon
  ...
  return (
    ...
    >
      <span className="min-w-0 flex-1 truncate">
        {sessionDisplayTitle(session, messages.untitledSession)}
      </span>
      <button
        type="button"
        data-session-delete
...
function EmptyState({ onPick }: { onPick: (text: string) => void }) {
  ...
  const suggestions = [
    {
      ...messages.suggestions.explainError,
      icon: SUGGESTION_ICONS.explainError,
    },
    {
      ...messages.suggestions.generateCommand,
      icon: SUGGESTION_ICONS.generateCommand,
    },
    {
      ...messages.suggestions.summarizeBuffer,
      icon: SUGGESTION_ICONS.summarizeBuffer,
    },
  ];
  return (
    <div className="flex flex-1 flex-col items-center justify-center gap-6 px-8 py-10 text-center">
      <img src="/logo.png" alt="Kite" className="size-14 opacity-90" />
