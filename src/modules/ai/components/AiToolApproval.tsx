...
function AiToolApprovalImpl({ part, toolName, onRespond }: Props) {
  const messages = useMessages().ai.approval;
  const meta = TOOL_META[toolName];
  const label =
    meta?.label && meta.label in messages.tools
      ? messages.tools[meta.label as keyof typeof messages.tools]
      : toolName;
    ...
    <div className="rounded-lg border border-border bg-card shadow-sm">
      <div className="flex items-center gap-2 border-b border-border/60 px-3 py-2">
        ...
        <span className="ml-auto text-[10px] text-muted-foreground">
          {messages.needsApproval}
        </span>
      </div>

      <div className="px-3 py-2.5">
        <PreviewBlock toolName={toolName} input={input} messages={messages} />
      </div>
...
function PreviewBlock({
  ...
  messages: ReturnType<typeof useMessages>["ai"]["approval"];
}) {
    ...
    return (
      <div className="space-y-0.5 font-mono text-[11px]">
        ...
        <div className="text-[10.5px] text-muted-foreground/80">
          {messages.lineCount(lines)} · {messages.reviewInDiffTab}
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="space-y-0.5 font-mono text-[11px]">
        <div className="text-muted-foreground">
          {String(input.path ?? "")}
          {input.replace_all ? ` · ${messages.replaceAll}` : ""}
        </div>
        <div className="text-[10.5px] text-muted-foreground/80">
          −{removed} / +{added} {messages.lineCount(Math.max(added, removed))} ·
          {messages.reviewInDiffTab}
        </div>
      </div>
    );
  }
  if (toolName === "multi_edit") {
    ...
    return (
      <div className="space-y-0.5 font-mono text-[11px]">
        <div className="text-muted-foreground">{String(input.path ?? "")}</div>
        <div className="text-[10.5px] text-muted-foreground/80">
          {messages.editCount(edits.length)} · {messages.reviewInDiffTab}
        </div>
      </div>
    );
  }
