    ...
    <div className="flex flex-wrap items-center gap-1.5">
      ...
      {commands.map((cmd) => (
        <Chip
          ...
          icon={cmd.icon}
          title={
            cmd.name in aiMessages.slashCommands
              ? aiMessages.slashCommands[
                  cmd.name as keyof typeof aiMessages.slashCommands
                ]
              : cmd.label
          }
          onRemove={() => onRemoveCommand(cmd.name)}
          removeLabel={messages.removeCommand}
        >
          #{cmd.name}
        </Chip>
      ))}
      {snippets.map((s) => (
        <Chip
          ...
          removeLabel={messages.removeSnippet}
        >
          {s.handle}
        </Chip>
      ))}
