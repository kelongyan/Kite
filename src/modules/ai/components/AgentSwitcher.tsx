...
export function AgentSwitcher({ isMiniWindow }: { isMiniWindow?: boolean }) {
  const messages = useMessages().ai.agentSwitcher;
  ...
  const displayAgent = (agent: (typeof list)[number]) =>
    agent.builtIn && agent.id in messages.builtInAgents
      ? messages.builtInAgents[
          agent.id as keyof typeof messages.builtInAgents
        ]
      : { name: agent.name, description: agent.description };
  const activeDisplay = displayAgent(active);
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          ...
          title={messages.agentTitle(activeDisplay.name)}
        >
