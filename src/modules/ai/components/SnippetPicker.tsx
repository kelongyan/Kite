...
export function SnippetPickerContent({
  ...
  const aiMessages = useMessages().ai;
  const messages = aiMessages.pickers;
    ...
    >
      {items.length === 0 ? (
        <div className="px-3 py-2.5 text-[11px] text-muted-foreground">
          {messages.noSnippetMatches}
        </div>
      ) : (
        <div className="max-h-64 overflow-y-auto py-1">
          {commands.length > 0 && (
            <>
              <SectionHeader label={messages.prebuiltSnippets} />
              <ul>
                {commands.map((it) => {
                  cursor += 1;
                  const i = cursor;
                        ...
                        <span className="flex min-w-0 flex-1 flex-col">
                          <span className="flex items-center gap-1.5">
                            ...
                            <span className="font-medium">
                              {c.name in aiMessages.slashCommands
                                ? aiMessages.slashCommands[
                                    c.name as keyof typeof aiMessages.slashCommands
                                  ]
                                : c.label}
                            </span>
                          </span>
          ...
          {snippets.length > 0 && (
            <>
              <SectionHeader label={messages.snippets} />
              <ul>
                {snippets.map((it) => {
                  cursor += 1;
                  const i = cursor;
