...
export function PlanDiffReview() {
  const messages = useMessages().ai.planDiff;
      ...
      <div className="flex items-center justify-between border-b border-border/40 px-3 py-2">
        <div className="flex flex-col">
          ...
          <span className="text-[10.5px] text-muted-foreground">
            {messages.pendingChange(queue.length)}
          </span>
        </div>
    ...
    <li className="group/row overflow-hidden rounded-md border border-border/50 bg-card">
      <div className="flex items-start gap-2 px-2.5 py-1.5">
        <button
          ...
          aria-label={messages.toggleDiff}
        >
          ...
          {stats ? (
            <div className="mt-0.5 flex items-center gap-2 text-[10px] tabular-nums">
              ...
              <span className="text-muted-foreground">
                {item.kind === "multi_edit" ? messages.multiEdit : item.kind}
              </span>
            </div>
          ) : (
            <div className="mt-0.5 text-[10px] text-muted-foreground">
              {item.description ?? messages.createDirectory}
            </div>
          )}
        </div>
        <Button
