...
export function AiOpenButton({ onOpen }: { onOpen: () => void }) {
  ...
  return (
    <button
      ...
      title={messages.openAiAgent}
    >
      <span>{messages.openAiAgent}</span>
...
export function AiStatusBarControls() {
  ...
  return (
    <div className="flex items-center gap-0.5">
      ...
      <IconBtn
        title={messages.attachFileOrImage}
      ...
      {c.voice.supported && (
        <IconBtn
          title={
            !c.voice.hasKey
              ? messages.voiceNeedsKey(STT_PROVIDER_LABELS[c.voice.sttProvider])
              : c.voice.recording
                ? messages.stopAndTranscribe
      ...
      <Button
        onClick={closePanel}
        title={messages.closeAiPanel}
        size="xs"
        variant="ghost"
        aria-label={messages.closeAiPanel}
      ...
      <IconBtn
        title={miniOpen ? messages.miniWindowOpen : messages.openConversation}
        onClick={openMini}
        disabled={miniOpen}
      >
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          ...
          title={
            currentProviderHasKey
              ? messages.modelTitle(current.label)
              : messages.modelNoKey(current.label)
          }
        >
          {current.label}
          <HugeiconsIcon
      ...
      >
        ...
        <div className="flex items-center gap-2.5 border-b border-border/70 px-3 py-2.5">
          ...
          <input
            ...
            placeholder={messages.searchModels}
              ...
              (p) => (
                <ProviderPill
                  ...
                  title={
                    hasKeyFor(p.id)
                      ? p.label
                      : messages.providerNotConfigured(p.label)
                  }
          ...
          <div className="min-h-0 flex-1 overflow-y-auto py-1">
            ...
            {filtered.length === 0 ? (
              <div className="flex items-center justify-center px-4 py-10 text-xs text-muted-foreground/70">
                {tab === "favorites"
                  ? messages.noFavorites
                  : tab === "recent"
                    ? messages.noRecent
                    : messages.noModelsMatch}
              </div>
            ) : (
              filtered.map((m) => (
                <ModelRow
  ...
  return (
    ...
    >
      ...
      <span className="flex-1 truncate">
        {messages.configureProvider(p.label)}
      </span>
  ...
  return (
    ...
    >
      <button
        ...
        title={favorite ? messages.unfavorite : messages.favorite}
        className={cn(
...
function CapabilityBars({ caps }: { caps: ModelCapabilities }) {
  ...
  return (
    <div className="ml-auto flex items-center gap-1.5">
      <CapBar icon={BrainIcon} value={caps.intelligence} label={messages.intelligence} />
      <CapBar icon={FlashIcon} value={caps.speed} label={messages.speed} />
      <CapBar
        icon={CoinsDollarIcon}
        value={caps.cost}
        label={messages.affordability}
      />
    </div>
  );
}
