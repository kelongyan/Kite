...
let
  ...
  srcMap = {
    x86_64-linux = fetchurl {
      url = "https://github.com/kelongyan/Kite/releases/download/v${version}/Kite_${version}_amd64.deb";
...
stdenv.mkDerivation {
  ...
  installPhase = if stdenv.hostPlatform.isLinux then ''
    ...
    install -Dm755 usr/bin/kite $out/bin/kite

    wrapProgram $out/bin/kite \
      "''${gappsWrapperArgs[@]}" \
  ...
  meta = with lib; {
    description = "Open-source lightweight cross-platform terminal emulator";
    homepage = "https://github.com/kelongyan/Kite";
