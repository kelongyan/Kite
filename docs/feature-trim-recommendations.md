...
> **历史文档，已被实际实现取代。** 本文写作时的建议是保留 SFTP、Git UI、编辑器和
> Markdown 预览。后续 `5b29635 refactor: slim Kite down to a terminal-only app`
> 走了更激进的路线，把这四块连同 `git-history`、`secrets` 一起删掉，Kite 现在只剩
> 终端加轻量目录侧栏。下文的“必须保留”清单不代表当前产品边界，请以 `ROADMAP.md`
> 和 `TERAX.md` 为准。保留本文只为记录当时的分析方法和取证数据。
