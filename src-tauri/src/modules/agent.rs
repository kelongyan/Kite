...
fn hook_command(spec: &AgentSpec, event: &str) -> String {
    match spec.delivery {
        Delivery::TerminalSequence => format!(
            r#"[ -n "$TERAX_TERMINAL" ] && printf '{{"terminalSequence":"\\u001b]777;notify;Kite;{event}\\u0007"}}' || true"#
        ),
...
fn osc_command(agent: &str, event: &str) -> String {
    format!(
        r#"[ -n "$TERAX_TERMINAL" ] && printf '\033]777;notify;Kite;{agent};{event}\007' > /dev/tty; printf '{{}}'"#
    )
}
...
fn status_needle(spec: &AgentSpec, event: &str) -> String {
    match spec.delivery {
        Delivery::TerminalSequence => format!("notify;Kite;{event}"),
        Delivery::Osc => {
            #[cfg(unix)]
            {
                format!("notify;Kite;{};{event}", spec.agent)
            }
            #[cfg(windows)]
            {
...
fn conout_marker(agent: &str, event: &str) -> String {
    format!("\x1b]777;notify;Kite;{agent};{event}\x07")
}

// Windows has no /dev/tty: the hook calls `kite.exe __terax_notify ...` and we
...
mod tests {
    ...
    fn claude_adds_all_event_hooks_to_empty_config() {
        ...
        assert!(command(&out, "Notification", 0).contains("notify;Kite;attention"));
        assert!(command(&out, "Stop", 0).contains("notify;Kite;finished"));
        assert!(command(&out, "UserPromptSubmit", 0).contains("notify;Kite;working"));
    ...
    fn conout_marker_matches_detector_format() {
        ...
        assert_eq!(
            ...
            "\u{1b}]777;notify;Kite;gemini;attention\u{7}"
        );
    }
    ...
    fn codex_emits_four_field_dev_tty_marker() {
        ...
        assert!(stop.contains("notify;Kite;codex;finished"));
    ...
    fn gemini_uses_matcher_and_named_marker() {
        ...
        assert!(command(&out, "AfterAgent", 0).contains("notify;Kite;gemini;finished"));
        assert!(command(&out, "Notification", 0).contains("notify;Kite;gemini;attention"));
    }
