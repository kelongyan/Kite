...
// OSC 777 markers our agent hooks emit. `notify;Terax;` is kept for hooks
// written before the app was renamed to Kite.
const AGENT_MARKERS: &[&[u8]] = &[b"notify;Kite;", b"notify;Terax;"];
...
impl AgentDetector {
    ...
    fn handle_osc777<F: FnMut(Transition)>(&mut self, pt: &[u8], emit: &mut F) {
        if let Some(tail) = AGENT_MARKERS.iter().find_map(|marker| pt.strip_prefix(*marker)) {
...
mod tests {
    ...
    #[test]
    fn kite_marker_drives_status() {
        let mut d = AgentDetector::new();
        run(&mut d, &osc("133;C;claude"));
        assert_eq!(run(&mut d, &osc("777;notify;Kite;attention")), vec![Transition::Attention]);
        assert_eq!(run(&mut d, &osc("777;notify;Kite;working")), vec![Transition::Working]);
        assert!(run(&mut d, &osc("777;notify;Kite;working")).is_empty());
        assert_eq!(run(&mut d, &osc("777;notify;Kite;finished")), vec![Transition::Finished]);
    }

    #[test]
    fn kite_marker_auto_arms_without_preexec() {
        let mut d = AgentDetector::new();
        assert_eq!(
            run(&mut d, &osc("777;notify;Kite;attention")),
            vec![started("claude"), Transition::Attention]
        );
    }

    #[test]
    fn legacy_terax_marker_still_works() {
        let mut d = AgentDetector::new();
        assert_eq!(
    ...
    fn four_field_marker_self_arms_named_agent() {
        ...
        let mut d = AgentDetector::new();
        assert_eq!(run(&mut d, &osc("777;notify;Kite;codex;working")), vec![started("codex")]);
        let mut g = AgentDetector::new();
        assert_eq!(
            run(&mut g, &osc("777;notify;Kite;gemini;finished")),
    ...
    fn four_field_marker_ignores_unknown_agent() {
        let mut d = AgentDetector::new();
        assert!(run(&mut d, &osc("777;notify;Kite;evil;attention")).is_empty());
        // A known agent in the same chunk still works.
        assert_eq!(
            run(&mut d, &osc("777;notify;Kite;codex;attention")),
    ...
    fn four_field_marker_drives_status_after_preexec() {
        ...
        assert_eq!(run(&mut d, &osc("777;notify;Kite;gemini;attention")), vec![Transition::Attention]);
        assert_eq!(run(&mut d, &osc("777;notify;Kite;gemini;working")), vec![Transition::Working]);
        assert_eq!(run(&mut d, &osc("777;notify;Kite;gemini;finished")), vec![Transition::Finished]);
    }
