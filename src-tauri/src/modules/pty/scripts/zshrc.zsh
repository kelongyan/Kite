...
# zsh, so we shadow $? into `_kite_ret`.

{
  _kite_user_zdotdir="${KITE_USER_ZDOTDIR:-$HOME}"
  [ -f "$_kite_user_zdotdir/.zshrc" ] && source "$_kite_user_zdotdir/.zshrc"
...
if [[ -z "$__KITE_HOOKS_LOADED" ]]; then
  __KITE_HOOKS_LOADED=1
  ...
  _kite_precmd() {
    local _kite_ret=$?
    printf '\e]133;D;%s\e\\' "$_kite_ret"
    printf '\e]7;file://%s%s\e\\' "${HOST}" "$(_kite_urlencode "$PWD")"
  ...
  _kite_preexec() {
    ...
    [[ -n "$KITE_BLOCKS" ]] && _kite_block_seen=1
    local cmd="${1//[[:cntrl:]]/ }"
    printf '\e]133;C;%s\e\\' "${cmd[1,256]}"
  }

  if (( $+functions[add-zsh-hook] )); then
    add-zsh-hook precmd _kite_precmd
    add-zsh-hook preexec _kite_preexec
  fi
