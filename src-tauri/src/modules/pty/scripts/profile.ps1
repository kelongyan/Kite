# kite-shell-integration (PowerShell)
...
if ($global:__KITE_HOOKS_LOADED) { return }
$global:__KITE_HOOKS_LOADED = $true
...
if (Test-Path Function:prompt) {
    Copy-Item Function:prompt Function:__kite_user_prompt -Force -ErrorAction SilentlyContinue
}

function global:__kite_urlencode {
    param([string]$s)
...
function global:__kite_install_readline {
    if ($global:__kite_readline_done) { return }
    if (-not (Test-Path Function:PSConsoleHostReadLine)) { return }
    $global:__kite_readline_done = $true
    ...
    Copy-Item Function:PSConsoleHostReadLine Function:global:__kite_user_readline -Force
    function global:PSConsoleHostReadLine {
        try {
            $line = __kite_user_readline
        } catch {
            # Self-heal: restore the original reader so a broken wrapper can
            # never lock the shell out of input.
            Copy-Item Function:__kite_user_readline Function:global:PSConsoleHostReadLine -Force
            return ''
        }
        try {
...
function global:prompt {
    ...
    $original = if (Test-Path Function:__kite_user_prompt) {
        try { & __kite_user_prompt } catch { "PS $((Get-Location).Path)> " }
    } else {
